"""
Octant AI — Math Engine: Stochastic Models

Provides quantitative implementations for Geometric Brownian Motion,
Merton Jump-Diffusion, and Ornstein-Uhlenbeck (Mean Reversion) processes.
Includes path simulation and Cholesky-based portfolio correlation matrices.
"""
...
def fit_merton_jumps(returns: pd.Series, cond_vol: pd.Series, dt: float = 1/252) -> Optional[MertonParams]:
    """Identifies and models jumps based on a 3-sigma conditional volatility threshold."""
...
def simulate_gbm_paths(params: GBMParams, n_paths: int, n_steps: int, dt: float = 1/252) -> np.ndarray:
    """Generates an array of n_paths x n_steps standard GBM asset price paths."""
    try:
...
def simulate_merton_paths(
    ...
    """Generates an array of paths under the Merton Jump-Diffusion model."""
    try:
        Z = np.random.standard_normal((n_paths, n_steps))
...
def _nearest_pd(A: np.ndarray) -> np.ndarray:
    """Finds the nearest positive-definite matrix to input A (Higham 1988)."""
    B = (A + A.T) / 2
...
def compute_correlated_paths(
    ...
    """Generates correlated multivariate random walks via Cholesky decomposition."""
    n_assets = len(means)
